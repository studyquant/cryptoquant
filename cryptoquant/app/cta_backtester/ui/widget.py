...
from cryptoquant.trader.engine import MainEngine
from cryptoquant.trader.ui import QtCore, QtWidgets, QtGui
from cryptoquant.trader.ui.widget import BaseMonitor, BaseCell, DirectionCell, EnumCell
from cryptoquant.trader.ui.editor import CodeEditor
from cryptoquant.event import Event, EventEngine
from cryptoquant.chart import ChartWidget, CandleItem, VolumeItem
from cryptoquant.trader.utility import load_json, save_json
...
class BacktesterManager(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.trade_dialog = BacktestingResultDialog(
            self.main_engine, self.event_engine, "回测成交记录", BacktestingTradeMonitor
        )
        self.order_dialog = BacktestingResultDialog(
            self.main_engine, self.event_engine, "回测委托记录", BacktestingOrderMonitor
        )
        self.daily_dialog = BacktestingResultDialog(
            self.main_engine, self.event_engine, "回测每日盈亏", DailyResultMonitor
        )
...
class BacktestingSettingEditor(QtWidgets.QDialog):
    ...
    def __init__(self, class_name: str, parameters: dict):
        """"""
...
class OptimizationResultMonitor(QtWidgets.QDialog):
    ...
    def __init__(self, result_values: list, target_display: str):
        """"""
        super().__init__()
