...
class OptionAPI(Client):
    ...
    def revoke_order(self, underlying, order_id="", client_oid=""):
        if order_id:
            return self._request_without_params(
                POST, OPTION_CANCEL_ORDER + str(underlying) + "/" + str(order_id)
            )
        elif client_oid:
            return self._request_without_params(
                POST, OPTION_CANCEL_ORDER + str(underlying) + "/" + str(client_oid)
            )
    ...
    def revoke_orders(self, underlying, order_ids="", client_oids=""):
        ...
        return self._request_with_params(
            POST, OPTION_CANCEL_ORDERS + str(underlying), params
        )
    ...
    def amend_order(
        ...
        return self._request_with_params(
            POST, OPTION_AMEND_ORDER + str(underlying), params
        )

    def amend_batch_orders(self, underlying, amend_data):
        params = {"amend_data": amend_data}
        return self._request_with_params(
            POST, OPTION_AMEND_BATCH_ORDERS + str(underlying), params
        )

    def get_order_info(self, underlying, order_id="", client_oid=""):
        if order_id:
            return self._request_without_params(
                GET, OPTION_ORDERS + "/" + str(underlying) + "/" + str(order_id)
            )
        elif client_oid:
            return self._request_without_params(
                GET, OPTION_ORDERS + "/" + str(underlying) + "/" + str(client_oid)
            )

    def get_order_list(
        self, underlying, state, instrument_id="", after="", before="", limit=""
    ):
        params = {"state": state}
        if instrument_id:
        ...
        if limit:
            params["limit"] = limit
        return self._request_with_params(
            GET, OPTION_ORDERS + "/" + str(underlying), params, cursor=True
        )

    def get_fills(
        self, underlying, order_id="", instrument_id="", after="", before="", limit=""
    ):
        params = {}
        if order_id:
            params["order_id"] = order_id
        ...
        if limit:
            params["limit"] = limit
        return self._request_with_params(
            GET, OPTION_FILLS + str(underlying), params, cursor=True
        )
    ...
    def get_specific_position(self, underlying, instrument_id=""):
        ...
        if instrument_id:
            params["instrument_id"] = instrument_id
        return self._request_with_params(
            GET, OPTION_POSITION + str(underlying) + "/position", params
        )
    ...
    def get_ledger(self, underlying, after="", before="", limit=""):
        ...
        if limit:
            params["limit"] = limit
        return self._request_with_params(
            GET, OPTION_ACCOUNT + str(underlying) + "/ledger", params, cursor=True
        )
    ...
    def get_instruments(self, underlying, delivery="", instrument_id=""):
        ...
        if instrument_id:
            params["instrument_id"] = instrument_id
        return self._request_with_params(
            GET, OPTION_INSTRUMENTS + str(underlying), params
        )
    ...
    def get_instruments_summary(self, underlying, delivery=""):
        ...
        if delivery:
            params["delivery"] = delivery
        return self._request_with_params(
            GET, OPTION_INSTRUMENTS + str(underlying) + "/summary", params
        )
    ...
    def get_depth(self, instrument_id, size=""):
        ...
        if size:
            params["size"] = size
        return self._request_with_params(
            GET, OPTION_INSTRUMENTS + str(instrument_id) + "/book", params
        )
    ...
    def get_trades(self, instrument_id, after="", before="", limit=""):
        ...
        return self._request_with_params(
            GET,
            OPTION_INSTRUMENTS + str(instrument_id) + "/trades",
            params,
            cursor=True,
        )
    ...
    def get_specific_ticker(self, instrument_id):
        return self._request_without_params(
            GET, OPTION_INSTRUMENTS + str(instrument_id) + "/ticker"
        )
    ...
    def get_kline(self, instrument_id, start="", end="", granularity=""):
        ...
        # 按时间正序 即由开始时间到结束时间
        data = self._request_with_params(
            GET, OPTION_INSTRUMENTS + str(instrument_id) + "/candles", params
        )
